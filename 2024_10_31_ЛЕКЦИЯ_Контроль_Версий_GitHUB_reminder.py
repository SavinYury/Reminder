...
#            print(r_time)
            mb.showinfo(title="Успех", message=f"Напоминание установлено на {hour:02}:{minute:02}") # Информируем что напоминание установлено
...
def stop_music():
    global music
    if music: # Проверяем на наличие включенной музыки и только тогда ее останавливаем
        pygame.mixer.music.stop()
        music = False

music = False # Задаем значение False по признаку, что музыка не играет
